...
- **WHEN** 正確 chunk 是很長的規格表，答案關鍵字（如 `TPM 2.0`）出現在片段前段之後（如第 576 字，超過前段長度）
