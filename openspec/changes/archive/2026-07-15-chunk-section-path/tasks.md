...
- [x] 2.2 `src/adapters/vector/sqlite.js`:`add` 的 FTS 寫入與 `_rebuildFts` 改索引 `segmentForFts(docId\n title\n content)`(v3 起含文件名);`PRAGMA user_version` 版本戳觸發一次性重建
...
- [x] 3.2 使用者驗收:重啟後(FTS 自動重建)手測標題詞問題;重灌 C455/C208 後再驗完整效果(向量面)
