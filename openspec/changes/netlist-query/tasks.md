...
- [ ] 5.1 查詢結果超過門檻時截斷+摘要（總數、依前綴統計、前 N 個）
- [ ] 5.2 `trace` 起點為電源/地腳（名稱樣式 VDD/VCC/GND/VSS… + 節點數門檻）回警告與建議，不展開大量路徑

**✅ 階段五驗收（決定性，唯讀 100T）**
- [ ] 5.3 測試：查已知電源網（如 `VDD_0V95_RTL0`）→ 結果被截斷且含摘要統計與總數
- [ ] 5.4 測試：`trace U42.1`（電源腳）→ 回警告/建議，路徑數不爆量
