...
Netlist Query capability for the markdown-rag-chatbot. Enables querying a board's Allegro PST netlist (`pstxprt.dat`、`pstchip.dat`、`pstxnet.dat`) via LLM tool-calling backed by `netparse`, so the assistant can answer connectivity, part, pin, net, and trace questions about a specific project's circuit.
